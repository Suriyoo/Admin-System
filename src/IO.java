...
public class IO {
    ...
    Book bb = new Book("title", "authorF", "authorL",0,"firstName","lastName");

    public IO(SortedArrayList<User> user, SortedArrayList<Book> book) throws IOException {
    ...
    public void printBook() {
        ...
        for (int i = 0; i < book.size(); i++) { //Traverse the book array
            ...
            if (b.getStatus() == 0) { //output whether the book is on loan or not
                System.out.println((i + 1) + ". '" + b.getTitle() + "' " + author + " [borrowed by " + b.getFirstName() + " " + b.getLastName()+"]");
            } else {
                System.out.println((i + 1) + ". '" + b.getTitle()+ "' " +author+" [Available Now]");
            }
        }
    }
    ...
    private void borrowBook() throws IOException {
        ...
        if(c1 == false) {
            ...
            System.out.println("Please enter the author name of the book you want to borrow (eg. Joy Smith)");
            String bookAuthor = x.nextLine();
            for (int i = 0; i < book.size(); i++) { //Traverse the book array
                Book b = (Book)book.get(i);
                String author = b.getAuthorFirstName()+b.getAuthorLastName();
                int su = bookTitle.compareTo(b.getTitle()); //check if the book is valid
                int fu = bookAuthor.compareTo(author);
                if (su == 0 && fu == 0) {
            ...
            if (a1 == false) { //it can be borrowed by user now
                uu.uBorrowBook(user,userFirstName,userSurname); //update user arraylist if meets all requirements
                bb.bBorrowBook(book,bookTitle,bookAuthor,userFirstName,userSurname); //update book arraylist if meets all requirements
    ...
    private void returnBook() {
        ...
        if(c3 == false) {
            ...
            System.out.println("Please enter the author name of the book you want to return (eg. Joy Smith)");
            ...
            for (int i = 0; i < book.size(); i++) { //Traverse the book array
                ...
                int fu = bookAuthor1.compareTo(author);
                if (su == 0 && fu == 0) {
            ...
            if (a1 == false) {
                uu.uReturnBook(user,userFirstName1,userSurname1); //update user arraylist if meets all requirements
                bb.bAddBook(book,bookTitle1,bookAuthor1,userFirstName1,userSurname1); //update book arraylist if meets all requirements
            }
        }
}
